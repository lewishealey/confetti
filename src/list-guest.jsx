  ...
  renderList: function() {
			...
			if(this.props.user.events) {
	          ...
	          var eventOptions = Object.keys(this.props.user.events).map(function (key, i) {
	            return <Choice key={key} id={key} value={i} name={this.props.user.events[key] ? this.props.user.events[key].name : ""} handleChoice={this.handleChoice} active={this.props.guest && this.props.guest.events ? (this.props.guest.events[key] ? true : false) : ''} />
	          }.bind(this));
	        } else {
	      ...
	      return <div className="row">
		        ...
		        {this.props.attending == false &&
		          <div className="col-md-4 tar guest__links">
		            <a className="link" href={"/#/page/" + this.props.user.username + "/" + this.props.id + "?admin=true"} target="blank">Guest view</a>
